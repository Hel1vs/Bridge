...
for(icat in c(1)){
  ...
  for(iper in c(1,2)){
    if(iper == 2) {tt[2] <- "2030"}
    dtn <- filter(cdata, Category %in% catsnat, variable %in% vars, period %in% tt,region==j,model==i)  %>% group_by(Category,region,variable,period,unit) %>% summarize(value=mean(value)) %>% ungroup()
    ...
    ggplot()+
      ...
      ggtitle(paste(i,j,sep="-"))+ #paste(country,"- 2050 emissions in reference scenario vs. decarbonisation scenario")
