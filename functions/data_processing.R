...
process_data <- function(all,scens){
    ...
    all <- filter(all, !(period %in% c(1950,1955,1960,1965,1970,1975,1980,1985,1990,1995,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2011,2012,2013,2014,2016,2017,2018,2019,2021,2022,2023,2024,2026,2027,2028,2029,2031,2032,2033,2034,2036,2037,2038,2039,2041,2042,2043,2044,2046,2047,2048,2049,2051,2052,2053,2054,2056,2057,2058,2059,2061,2062,2063,2064,2066,2067,2068,2069,2071,2072,2073,2074,2076,2077,2078,2079,2081,2082,2083,2084,2086,2087,2088,2089,2091,2092,2093,2094,2096,2097,2098,2099,2101,2102,2103,2104,2106,2107,2108,2109)))
...
add_variables <- function(all,scens){
  ...
  # doable graphs for national paper
  ...
  #all <- calcVariable(all,'`Renewables Share|Excl. Nuclear` ~ ( `Secondary Energy|Electricity|Solar` + `Secondary Energy|Electricity|Wind` + `Secondary Energy|Electricity|Hydro` + `Secondary Energy|Electricity|Biomass` ) / `Secondary Energy|Electricity` * 100 ' , newUnit='%')
 ...
 all <- calcVariable(all,'`Share of Elec in FE` ~  `Final Energy|Electricity`   / `Final Energy` * 100 ' , newUnit='%')
  ...
  all <- calcVariable(all,'`Share of Elec in Industry` ~  `Final Energy|Industry|Electricity`   / `Final Energy|Industry` * 100 ' , newUnit='%')
  all <- calcVariable(all,'`Share of Elec in Resid Commerc` ~  `Final Energy|Residential and Commercial|Electricity`   / `Final Energy|Residential and Commercial` * 100 ' , newUnit='%')
  ...
  all <- calcBudget_2015(all,'Emissions|CO2','Carbon budget')
  all <- calcBudget_2015(all,'Emissions|CO2|Energy and Industrial Processes','Carbon budget|Energy and Industry')
  all <- calcBudget_2015(all,'Emissions|CO2|Energy','Carbon budget|Energy')
  # source("functions/calcBudget_2015.R")
